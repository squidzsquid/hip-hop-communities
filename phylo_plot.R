...
load("simplified_network_new.rds")
#load the parsed releases used to construct the TILES input file
data_clean <- read.csv("releases.tsv", sep="\t")
data_clean <- data_clean[grepl(",", data_clean$artists), ]
data_clean$artists <- lapply(data_clean$artists, function(x) unlist(strsplit(x, ", ")))
data_clean$styles <- lapply(data_clean$styles, function(x) unlist(strsplit(x, ", ")))
...
# N.B -- unsure what this is really meant to be doing, disabled for now
